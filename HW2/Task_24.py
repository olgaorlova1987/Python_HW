...
list_a = list (randint(1,10) for i in range (int (input ('Введите количество кустов: '))))
print (list_a)

maximum_berries = 0

for i in range (len(list_a)):
    berries_i = list_a[i-2]+ list_a [i-1] +list_a [i]
    berries_i = list_a[i-1]+ list_a[i] +list_a[(i+1)% len(list_a)]
    berries_i = list_a[i-1]+ list_a[i] +list_a[i+1-len(list_a)]
    if berries_i > maximum_berries:
        maximum_berries = berries_i
